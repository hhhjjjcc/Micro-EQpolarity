...
def read_hash3_sta_corr(corfile):
    ...
    stacor_df['station'] = stacor_df['station'].astype(str)
    return stacor_df
...
def read_amp_file(ampfile,input_format,merge_on,ratmin,min_sp,max_sp):
    ...
    # Aug. 21, 2024, YC, ensure the type of station column is string
    spamp_df['station'] = spamp_df['station'].astype(str) 
...
def read_hash3_amp_file(ampfile):
    ...
    for line in file1:
	    ...
	    # Aug. 20, 2024, YC
	    # check number of fields in this line
            if len(split_line) == 9:
            ...
            if len(split_line) ==7:
                ...
                cha.append(split_line[1])
                net.append(split_line[2])
                noise_p.append(split_line[3])
                noise_s.append(split_line[4])
                amp_p.append(split_line[5])
                amp_s.append(split_line[6])
                loc.append('--')
    if len(sta)>0:
        tmp_df=pd.DataFrame(data=np.asarray(np.vstack((sta,net,loc,cha,noise_p,noise_s,amp_p,amp_s))).T,
                            columns=['station','network','location','channel','noise_p','noise_s','amp_p','amp_s'])
