...
# File path
file_path = "mt_solution_group3_20250319.txt"

# Generate dynamic filename
fault_type = "reverse_fault"
...
strikes = np.array([event['strike'] for event in events])
dips = np.array([event['dip'] for event in events])
rakes = np.array([event['rake'] for event in events])

avg_strike = np.mean(strikes)
std_strike = np.std(strikes)
...
filtered_events = [
    ...
    if (avg_strike - std_threshold * std_strike <= event['strike'] <= avg_strike + std_threshold * std_strike) and
...
if filtered_events:
    strikes_filter = np.array([event['strike'] for event in filtered_events])
    ...
    print(f"Filtered Average Rake: {avg_rake_filter:.2f}")
else:
...
for event in filtered_events:
    ...
    color = (normalized_deviation, 0.8, normalized_deviation)  # RGB color definition
...
# For Stress
# Sigma 1
x1, y1 = stereonet_projection(53, 26)
plt.scatter(x1, y1, color='purple', marker='o', s=200, edgecolor='black', linewidth=1, zorder=5)

# Sigma 2
x2, y2 = stereonet_projection(288, 51)
plt.scatter(x2, y2, color='green', marker='s', s=200, edgecolor='black', linewidth=1, zorder=5)

# Sigma 3
x3, y3 = stereonet_projection(158, 28)
plt.scatter(x3, y3, color='yellow', marker='^', s=200, edgecolor='black', linewidth=1, zorder=5)

# Add legend
legend_elements = [
    plt.scatter([], [], color='purple', edgecolor='black', linewidth=1, marker='o', s=100, label='σ1 (53°/26°)'),
    plt.scatter([], [], color='green', edgecolor='black', linewidth=1, marker='s', s=100, label='σ2 (288°/51°)'),
    plt.scatter([], [], color='yellow', edgecolor='black', linewidth=1, marker='^', s=100, label='σ3 (158°/28°)')
]
...
# Create RGB string for the file name
rgb_str = f"{round(color[0], 2)}_{round(color[1], 2)}_{round(color[2], 2)}"
current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
output_filename = f"./{fault_type}_beachball_plot_std_{std_threshold}_{current_time}_{rgb_str}_nodal1.png"
